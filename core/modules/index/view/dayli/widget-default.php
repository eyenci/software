...
$spents = OperationData::getAllByDate(date("Y-m-d",time()));
}else{
$spents = OperationData::getAllByDateAndCategoryId(date("Y-m-d",time()),$_GET["cat_id"]);
$category = CategoryData::getById($_GET["cat_id"]);
}
$money = 0;
 foreach($spents as $career){ $money += $career->q*$career->price; }?>
...
<div class="panel panel-default">
  ...
  <a class='list-group-item' href="index.php?view=dayli"><?php if($category==null): ?><i class="glyphicon glyphicon-ok"></i> <?php endif; ?>Todas</a>
<?php foreach(CategoryData::getAll() as $cat):?>
  <a class='list-group-item' href="index.php?view=dayli&cat_id=<?php echo $cat->id; ?>"><?php if( $category!=null && ($category->id==$cat->id)): ?><i class="glyphicon glyphicon-ok"></i> <?php endif; ?><?php echo $cat->name; ?></a>
<?php endforeach; ?>
	</div>
</div>
</div>
...
<tr>
  ...
  <td style=""><?php echo " $ ".($career->price); ?></td>
	<td style="width:100px;"><?php echo " $ ".($career->q*$career->price); ?></td>
	<td><?php echo $career->cname; ?></td>
...
<script>
	$("#del-<?php echo $career->id?>").click(function(){
		...
		if(c==true){
			window.location = "index.php?view=delopx&id=<?php echo $career->id; ?>";
		}
	});
</script>
	</td>
...
<?php else: // no careers ?>
<div class="jumbotron">
	<h2><i class="glyphicon glyphicon-minus-sign"></i> No hay Ventas </h2>
</div>
<?php endif; ?>
	</div>
</div>
